...
export default function DashboardStatus() {
  ...
  const authoritative=useAuthoritativeTradingState();
  const validated=betfairConnection?.apiValidationStatus==='api_connected'||markets.length>0;
  const priced=authoritative.priceFeedStatus==='LIVE';
  ...
  const cards = [
    ...
    {label:'Market Data',value:priced?`LIVE · ${authoritative.priceAgeSeconds}s old`:markets.length?authoritative.priceFeedStatus:'UNAVAILABLE',icon:Radio,ok:priced},
