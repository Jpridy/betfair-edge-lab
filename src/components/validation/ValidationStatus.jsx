...
export default function ValidationStatus(){const{statisticalValidation:v}=useApp();return <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-card p-4"><div><div className="text-xs font-semibold text-foreground">Statistical validation</div><div className="mt-1 text-[11px] text-muted-foreground">{v.settledBets}/500 eligible settled bets · test set {v.testBets}/100</div></div><StatusBadge status={tone(v.status)}>{v.status.replaceAll('_',' ')}</StatusBadge></div>}