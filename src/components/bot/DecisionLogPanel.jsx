...
export const CYCLE_EXPORT_COLUMNS = [
  ...
  { key: 'priceAgeSeconds', label: 'PriceAgeSeconds' },
  { key: 'staleThresholdSeconds', label: 'StaleThresholdSeconds' },
  { key: 'authoritativePriceTimestamp', label: 'AuthoritativePriceTimestamp' },
  { key: 'aiCacheUsed', label: 'AICacheUsed' },
  { key: 'nextRaceWindowOpensAt', label: 'NextRaceWindowOpensAt' },
  { key: 'debugScanMode', label: 'DebugScanMode' },
  { key: 'globalMarketsLoaded', label: 'GlobalMarketsLoaded' },
  { key: 'globalMarketsOpen', label: 'GlobalMarketsOpen' },
  { key: 'globalMarketsWithRunners', label: 'GlobalMarketsWithRunners' },
  { key: 'globalMarketsWithPrices', label: 'GlobalMarketsWithPrices' },
  { key: 'selectedRaceMarketsLoaded', label: 'SelectedRaceMarketsLoaded' },
  { key: 'selectedRaceMarketsInsideWindow', label: 'SelectedRaceMarketsInsideWindow' },
  { key: 'selectedRaceMarketsEligible', label: 'SelectedRaceMarketsEligible' },
  { key: 'selectedRaceMarketsSentToEngine', label: 'SelectedRaceMarketsSentToEngine' },
  ...
  { key: 'finalProbabilityUsedInEV', label: 'FinalProbabilityUsedInEV' },
  ...
  { key: 'aiRunnerCountReturned', label: 'AIRunnerCountReturned' },
...
export function cycleToRow(c) {
  ...
  const race = ss.opportunityFunnel || {};

  return {
    cycleId: String(c.cycleId || c.id || ''),
    ...
    priceAgeSeconds: num(cd.priceAgeSeconds),
    staleThresholdSeconds: num(cd.staleThresholdSeconds),
    authoritativePriceTimestamp: cd.authoritativePriceTimestamp || '',
    aiCacheUsed: race.aiCacheUsed ? 'TRUE' : 'FALSE',
    nextRaceWindowOpensAt: race.nextRaceWindowOpensAt || '',
    debugScanMode: ss.debugScanMode ? 'TRUE' : 'FALSE',
    globalMarketsLoaded: ss.globalMarketsLoaded ?? mfd.marketsInMemory ?? 0,
    globalMarketsOpen: ss.globalMarketsOpen ?? mfd.marketsOpen ?? 0,
    globalMarketsWithRunners: ss.globalMarketsWithRunners ?? mfd.marketsWithRunners ?? 0,
    globalMarketsWithPrices: ss.globalMarketsWithPrices ?? mfd.marketsWithPriceData ?? 0,
    selectedRaceMarketsLoaded: ss.selectedRaceMarketsLoaded ?? 0,
    selectedRaceMarketsInsideWindow: ss.selectedRaceMarketsInsideWindow ?? 0,
    selectedRaceMarketsEligible: ss.selectedRaceMarketsEligible ?? 0,
    selectedRaceMarketsSentToEngine: ss.selectedRaceMarketsSentToEngine ?? 0,
    ...
    marketId: String(bc.betfairMarketId || bc.marketId || ''),
    ...
    finalProbabilityUsedInEV: num(bc.finalProbabilityUsedInEV ?? bc.modelProbability),
    ...
    aiRunnerCountReturned: ss.aiRunnerCountReturned ?? ss.aiObservability?.at(-1)?.aiRunnerCountReturned ?? ss.aiObservability?.at(-1)?.aiResponseRunnerCount ?? 0,
    orderCreated: c.ordersCreated ?? 0,
    settlementStatus: c.settlementStatus || (c.ordersCreated > 0 ? 'awaiting_result' : 'not_applicable'),
    externalSearchUsed: bc.externalSearchUsed ? 'TRUE' : 'FALSE',
    externalSearchStatus: bc.externalSearchStatus || 'not_requested',
...
function opportunitiesToRows(cycle) {
  ...
  topOpps.forEach((opp, i) => {
    rows.push({
      ...
      selectionId: String(opp.selectionId || ''),
      ...
      finalProbabilityUsedInEV: num(opp.finalProbabilityUsedInEV),
      ...
      externalSearchStatus: opp.externalSearchStatus || 'not_requested',
