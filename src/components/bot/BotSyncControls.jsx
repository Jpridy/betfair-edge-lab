...
export default function BotSyncControls() {
  ...
  const connectionChecks = [
    ...
    { label: 'Markets Synced', ok: markets.length > 0 },
    ...
    <Panel title="Betfair API Connection & State Controls">
      <div className="p-4 space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          ...
          <div className="flex items-center gap-1 text-muted-foreground">
            <Zap className="h-3 w-3" /> Stream: <span className="font-mono text-foreground">{isStreaming ? 'LIVE' : 'OFF'}</span>
          </div>
        </div>

        {betfairConnection?.lastMarketSyncTime && (
          <div className="text-xs text-muted-foreground border-t border-border pt-2">
            Last sync: <span className="font-mono">{new Date(betfairConnection.lastMarketSyncTime).toLocaleString('en-AU')}</span>
          </div>
        )}
      </div>
    </Panel>
