...
export function cycleToRow(cycle) {
  ...
  return {
    cycleId:String(cycle.cycleId || cycle.id || ''), cycleNumber:cycle.cycleNumber ?? '', cycleRunKey:cycle.cycleRunKey || '', schedulerInstanceId:cycle.schedulerInstanceId || '', browserTabId:cycle.browserTabId || '', triggerSource:cycle.triggerSource || '', timestamp:cycle.finishedAt || cycle.startedAt || '',
    ...
    failedGate:cycle.ordersCreated > 0 ? '' : (summary.cycleFailedGate || summary.failedGate || 'NO_VALID_CANDIDATE'), noBetReason:cycle.noBetReason || summary.noBetReason || '', orderCreated:cycle.ordersCreated || 0,
