...
const allRunners=[...winPrices.map(r=>runner('verify-win',...r)),...winPrices.map(r=>runner('verify-place',...r)),runner('verify-h2h',1,'Alpha',1.9,1.92,100,100),runner('verify-h2h',2,'Bravo',2.08,2.1,100,100)];
const aiResult={runnerProbabilities:[{selectionId:'1',pWin:.56,pPlace:.8,confidence:85},{selectionId:'2',pWin:.18,pPlace:.55,confidence:85},{selectionId:'3',pWin:.1,pPlace:.4,confidence:75},{selectionId:'4',pWin:.06,pPlace:.3,confidence:70},{selectionId:'5',pWin:.04,pPlace:.22,confidence:65},{selectionId:'6',pWin:.03,pPlace:.16,confidence:60},{selectionId:'7',pWin:.02,pPlace:.1,confidence:55},{selectionId:'8',pWin:.01,pPlace:.07,confidence:50}],h2hProbabilities:[{marketId:'verify-h2h',selectionId:'1',opponentSelectionId:'2',pBeatsOpponent:.44,confidence:80}],dataQuality:85,confidence:85,raceSummary:'Structured selection probabilities'};
const settings={defaultTimeWindowStartSeconds:500,defaultTimeWindowEndSeconds:30,baseStake:2,maxStake:5,paperBankroll:10000,maxLayLiability:1500,maxMarketExposure:1000,maxOpenOrders:10,dailyLossLimit:500,allowInPlay:false,defaultCommissionRate:.05};
...
async function run(orders=[]){clearRaceDayCache();clearRacePackAiCache();return runExchangeCycle({markets,allMarkets:markets,runners:allRunners,settings,botSettings:{selectedStrategies:['Featherless AI Value Decision Engine']},featherlessSettings,bankrollStats:{bankroll:10000,available:10000,openPaperExposure:0},paperOrders:orders,connectionState:{apiConnected:true,lastActualPriceUpdateAt:new Date().toISOString()},callAI:async()=>aiResult,maxEventsToScan:1});}

describe('critical repair verification',()=>{
  it('proves grouping, side-neutral selection and race exposure blocking',async()=>{
    const first=await run();
    const selected=first.bestOpportunity;
    ...
    expect(first.diagnostics.sideSelectionDiagnostics.bestBackOpportunityId).toBeTruthy();
    expect(first.diagnostics.sideSelectionDiagnostics.bestLayOpportunityId).toBeTruthy();
    const existing={eventId:'critical-verification-race',marketId:selected.marketId,betfairMarketId:selected.betfairMarketId,status:'matched',settlementStatus:'awaiting_result',selectionId:selected.selectionId,side:selected.side};
