...
export async function authorizeAndCreatePaperOrder(context = {}) {
  ...
  const order={...validated.order,rawModelProbability:opportunity.rawModelProbability??opportunity.modelProbability,calibratedProbability:opportunity.calibratedProbability??opportunity.finalProbabilityUsedInEV??opportunity.modelProbability,calibrationModelVersion:opportunity.calibrationModelVersion??'raw-v1',finalProbabilityUsedInEV:opportunity.finalProbabilityUsedInEV??opportunity.modelProbability,rankedCalculation,finalAuthorityRecalculation:recomputedCalculation,valuesMatched:true,finalAuthorityReached:true,rawCommissionRate:commission.rawRate,normalizedCommissionRate:commission.normalizedRate,commissionSource:commission.source,commissionNormalizationApplied:commission.normalizationApplied,priceFeedStatus:freshness.priceFeedStatus,priceAgeSeconds:freshness.priceAgeSeconds,paperSimulationQuality:freshness.priceFeedStatus==='LIVE'?validated.order.paperSimulationQuality:'Basic'};
