...
describe('V9 absolute order authority',()=>{
  ...
  it('invalid sides cannot create an order',async()=>{const c=base();c.opportunity.side='BUY';expect((await authorizeAndCreatePaperOrder(c)).failedGate).toBe('INVALID_SIDE');expect(c.entityApi.create).not.toHaveBeenCalled();});
  ...
  it('rejects a price move before persistence',async()=>{const c=base();c.runner.bestBackPrice=2.04;expect((await authorizeAndCreatePaperOrder(c)).failedGate).toBe('PRICE_MOVED');expect(c.entityApi.create).not.toHaveBeenCalled();});
  it('rechecks frozen market ROI thresholds',async()=>{const c=base();c.opportunity.requiredMinROI=50;expect((await authorizeAndCreatePaperOrder(c)).failedGate).toBe('ROI_BELOW_MINIMUM');});
  it('validates H2H against the matching AI pair',async()=>{const c=base();const calculationResult=buildCalculationResult({side:'BACK',probability:.6,odds:2.02,normalizedCommissionRate:.05,stake:2});c.opportunity={...c.opportunity,...calculationResult,marketType:'H2H',opponentSelectionId:'2',baseProbability:.6,maxLoss:calculationResult.lossIfLose,calculationResult};c.strategyRequiresAI=true;c.aiResult={runnerProbabilities:[{selectionId:'1',pWin:.2}],h2hProbabilities:[{marketId:'m1',selectionId:'1',opponentSelectionId:'2',pBeatsOpponent:.6}]};expect((await authorizeAndCreatePaperOrder(c)).authorized).toBe(true);});
});
