...
describe('V9 absolute order authority',()=>{
  ...
  it('debug mode is read only and retains candidate calculations',async()=>{const c=base();c.debugMode=true;const r=await authorizeAndCreatePaperOrder(c);expect(r.failedGate).toBe('DEBUG_MODE_READ_ONLY');expect(c.opportunity.ev).toBe(1);expect(c.entityApi.create).not.toHaveBeenCalled();});
});
