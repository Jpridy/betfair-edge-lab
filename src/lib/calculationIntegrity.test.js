...
describe('calculation integrity primitives',()=>{
 ...
 it('uses commission-adjusted Kelly and permits zero',()=>{expect(calcKellyStake(.4,2,1000,1,.25,.05).stake).toBe(0);expect(calcKellyStake(.6,2,1000,1,.25,.05).stake).toBeLessThan(calcKellyStake(.6,2,1000,1,.25,0).stake);expect(calcLayKellyStake(.6,2,1000,1,.25,.05).stake).toBe(0);expect(calcLayKellyStake(.1,4,10000,1,1,.05).stake).toBeCloseTo(1947.368421,5);});
 it('hard rejects invalid bet sides',()=>{expect(buildCalculationResult({side:'BUY',probability:.5,odds:2,normalizedCommissionRate:.05,stake:10})).toMatchObject({mathematicalInvariantsPassed:false,error:'INVALID_SIDE'});});
 ...
 it('WIN totals one and awaiting result remains exposure',()=>{expect(normalizeWinProbabilities([{selectionId:'1',pWin:.7},{selectionId:'2',pWin:.3}]).reduce((s,x)=>s+x.pWin,0)).toBeCloseTo(1,12);expect(calculateRiskMetrics([{side:'LAY',status:'awaiting_result',requestedStake:2,requestedOdds:4}],{}).openExposure).toBe(6);});
});