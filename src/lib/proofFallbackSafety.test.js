...
describe('paper proof fallback hard gates', () => {
  it('does not create proof fallback when prices are unavailable', () => {
    expect(getProofFallbackHardGate(context({ priceFeedStatus: 'UNAVAILABLE' }))).toBe('PRICE_DATA_UNAVAILABLE');
    expect(build([], context({ priceFeedStatus: 'UNAVAILABLE' }))).toBeNull();
  });

  it('does not create proof fallback when the race is locked', () => {
    const locked = { ...unlocked, raceLocked: true, activeOrderExistsForRace: true };
    expect(build([], context({ raceMonitoring: locked }))).toBeNull();
  });

  it('does not create a second order while awaiting a result', () => {
    expect(build([{ id: 'o1', eventId: 'e1', status: 'awaiting_result' }])).toBeNull();
  });

  it('creates an explicitly labelled proof override only when every hard gate passes', () => {
    expect(build()).toMatchObject({
      decision: 'PROOF_OVERRIDE',
      decisionSource: 'PROOF_OVERRIDE',
      proofMode: true,
  ...
  it('keeps PLACE proof orders blocked', () => {
    const place = { ...market, id: '1.200', betfairMarketId: '1.200', marketTypeCode: 'PLACE', marketName: 'Place', numberOfWinners: 2 };
    const placeCluster = { ...cluster, winMarkets: [], placeMarkets: [place] };
    const placeRunners = runners.map(item => ({ ...item, marketId: '1.200' }));
    expect(buildProofOpportunity([placeCluster], placeRunners, [], settings, context())).toBeNull();
  });
});

describe('accepted engine market diagnostics', () => {
  it('marks the market sent to the engine as accepted', () => {
    const selectedRace = { ...cluster, raceKey: 'e1', markets: [market] };
    const report = buildRaceMonitoringDiagnostics({ selectedRace, runners, acceptedMarketIds: ['1.100'], now });
    expect(report.selectedRaceMarketDetails).toEqual([expect.objectContaining({ normalizedMarketId: '1.100', accepted: true })]);
  });
});
